...
public class EarthquakesInteractorTest {
  ...
  public void firstCachedThenActualValuesReturned() throws Exception {
    when(earthquakesRepository.getCachedTodaysEarthquakes())
        .thenReturn(Single.just(new ArrayList<>()));
    when(earthquakesRepository.getTodaysEarthquakesFromApi())
        .thenReturn(Single.just(new ArrayList<>()));
  ...
  public void earthquakeReturnedIfItPassesSettings() throws Exception {
    when(earthquakesRepository.getTodaysEarthquakesFromApi())
        .thenReturn(Single.just(Collections.singletonList(testEarthquake)));
    ...
    verify(earthquakesRepository).getTodaysEarthquakesFromApi();
    testObserver.assertValue(o -> ((EntitiesWrapper<EarthquakeWithDist>) o).getState()
        .equals(EntitiesWrapper.State.SUCCESS));
  }
  ...
  public void earthquakeNotReturnedIfItDoesNotPassMaxDistanceSettings() throws Exception {
    ...
    verify(earthquakesRepository).getTodaysEarthquakesFromApi();
    testObserver.assertValue(o -> ((EntitiesWrapper<EarthquakeWithDist>) o).getState()
        .equals(EntitiesWrapper.State.EMPTY));
  }
