...
public class AlertPresenterTest {
  ...
  public void setUp() throws Exception {
    ...
    when(locationInteractor.getCurrentCoordinates()).thenReturn(
        Single.just(
            new LocationInteractor.LocationAnswer(testCoords, LocationInteractor.State.SUCCESS)));
    when(locationInteractor.checkLocationServicesAvailability()).thenReturn(Single.just(true));
    when(earthquakesInteractor.getEarthquakeAlert(testCoords)).thenReturn(Single
        .just(new EntitiesWrapper<>(EntitiesWrapper.State.SUCCESS, new EarthquakeWithDist())));
    when(schedulersProvider.ui()).thenReturn(Schedulers.trampoline());
    alertPresenter = new AlertPresenter(earthquakesInteractor, locationInteractor,
        settingsInteractor, schedulersProvider);
